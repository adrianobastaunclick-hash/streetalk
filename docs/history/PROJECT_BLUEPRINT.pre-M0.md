# Documento storico, superato dal PROJECT_BLUEPRINT.md corrente

Fotografia delle modifiche locali prima di M0. Affermazioni e istruzioni sottostanti non sono requisiti correnti né prove di verifica.
...
- **RAM Realtime Architecture & Matchmaking FIFO**: Coda di accoppiamento deterministica in-memory separata per mood (cazzeggio, sfogati, flirt) con filtraggio opzionale per genere. I messaggi della chat non vengono persistiti; i segreti e le segnalazioni possono invece essere archiviati in Supabase quando configurato.
...
- **Due modalità operative**: senza `SUPABASE_URL` e relativa chiave il servizio opera solo in RAM e non effettua scritture cloud; con credenziali valide mantiene lo stato realtime in RAM e abilita la persistenza Supabase, senza bloccare il flusso in caso di errore di archiviazione.
- **Contratto di archiviazione cloud**: `createRoom` chiama `archiveSecret` al match per ogni utente e salva testo integrale, mood, data, ID, contatore like e flag pubblico (attualmente `true`) per il feed pubblico `/api/secrets`. `report_user` chiama `logReport` soltanto per una segnalazione valida con IP del partner e salva `roomId`, motivo, stato, data e hash abbreviati degli IP per finalità di moderazione; le segnalazioni non sono esposte dall'API pubblica.
- **Conservazione**: nessuna scadenza automatica è definita nello schema Supabase, quindi segreti e report cloud restano finché non vengono eliminati amministrativamente. La distruzione di stanze e timer riguarda esclusivamente la copia in RAM.
